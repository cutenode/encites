...
async function filterPublicGitHubEvents (events) {
  ...
  for (const singleEvent in events) {
    ...
    if (event.type === 'IssueCommentEvent' && event.payload.action === 'created') {
      ...
      eventDataToReturn.commentCreatedAt = event.payload.comment.created_at
      eventDataToReturn.commentAuthorAssociation = event.payload.comment.author_association
