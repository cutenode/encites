...
async function getMarkdownFromEvents (events, title) {
  ...
  for (const event of events) {
    ...
    if (event.type === 'IssueCommentEvent') {
      ...
      logger(__filename, `incrementing counts.commentsCreated (current value: ${counts.commentsCreated}) counter`)
      counts.commentsCreated = counts.commentsCreated + 1
      logger(__filename, `incremented counts.commentsCreated (current value: ${counts.commentsCreated}) counter`)

      const bullet = `* ${author} created a comment on ${link} in ${repo}: #${event.payload.issue.number} (comment)`
      data.prBullets.push(bullet)
  ...
  const document = `${heading}${countSection}${contributorsSection}${issueSection}${prSection}${commentSection}`
